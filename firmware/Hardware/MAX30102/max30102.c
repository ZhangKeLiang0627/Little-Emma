...
extern uint32_t ppg_data_cache_RED[CACHE_NUMS]; // 缓存区
extern uint32_t ppg_data_cache_IR[CACHE_NUMS];  // 缓存区
...
u8 max30102_Bus_Write(u8 Register_Address, u8 Word_Data)
{
	...
	IIC_Send_Byte_MAX30102(max30102_WR_address | I2C_WR);	/* 此处是写指令 */
...
u8 max30102_Bus_Read(u8 Register_Address)
{
	...
	IIC_Send_Byte_MAX30102(max30102_WR_address | I2C_RD);	/* 此处是读指令 */
	...
	/* 第9步：读取数据 */
	{
		data = IIC_Read_Byte_MAX30102(0);	/* 读1个字节 */

		IIC_NAck_MAX30102();	/* 最后1个字节读完后，CPU产生NACK信号(驱动SDA = 1) */
	}
	/* 发送I2C总线停止信号 */
	IIC_Stop_MAX30102();
...
//	max30102_Bus_Write(REG_LED2_PA, 0x47); 
	...
	max30102_Bus_Write(REG_INTR_ENABLE_1,0xd0);	// INTR setting // 启用外部中断引脚
...
uint8_t peakfound_flag = 0; // 找到波峰标志位

uint8_t fingerfound_flag = 0; // 导联标志位/是否有手指按下

uint16_t i; // 计数值

extern uint32_t Time_ms;

float ir_ppgtemp, ir_ppgtemp_fir; // ir的ppg信号

uint8_t MAX30102_Get_DATA(int32_t *HeartRate,int32_t *SpO2,uint32_t max30102_data[2])
{	
	
	if(max30102_set_flag) // 中断信号产生
	{
		max30102_set_flag = 0;
		max30102_fifo_read(max30102_data); // 读取数据
		
		ir_ppgtemp = (float)max30102_data[0];
		ir_max30102_fir(&ir_ppgtemp, &ir_ppgtemp_fir); // 滤波
		...
		if(max30102_data[0] > PPG_DATA_THRESHOLD && max30102_data[1] > PPG_DATA_THRESHOLD)
		{
			...
			fingerfound_flag = 1; // 导联标志位置1/有手指按下
			
			if(un_max > ir_ppgtemp_fir) // 找波峰
			{
				peakfound_flag = 1;
				un_max = 0;
			}
...
//			printf("IRac:%d   REDac:%d\r\n", max30102_data[0], max30102_data[1]);
			...
			printf("@%d#", max30102_data[0]); // 发送给上位机的ppg波形
			...
			if (cache_counter >= BUFFER_SIZE) // 收集满数据
			{
				...
				if(ch_hr_valid == 1 && ch_spo2_valid == 1)
				{
					...
					*HeartRate = n_heart_rate;
					...
					printf("$%d,%d^",*HeartRate,*SpO2); // 发送给上位机的数值
				}
				else
				{
					*SpO2 = 0;
		...
		else // 数据低于阈值，数值为零
		{
			...
			*HeartRate = 0;
			cache_counter = 400;
			fingerfound_flag = 0; // 导联标志位置0/无手指按下
		}
